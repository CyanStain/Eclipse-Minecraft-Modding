...
import com.cyanstain.tutorialmod.objects.items.SpecialItem;
...
public class ItemInit {
	...
	// food items
	public static final Item foot_lettuce = null;
	// special needs items
	...
	public static void registerItems(final RegistryEvent.Register<Item> event) {
		...
		event.getRegistry()
				.register(new Item(new Item.Properties().group(TutorialItemGroup.instance)
						.food(new Food.Builder().hunger(3).saturation(1.2f)
								...
								.effect(() -> new EffectInstance(Effects.HUNGER, 200, 0), 0.9f).build()))
										.setRegistryName("foot_lettuce"));
		// these aren't any ordinary items, these are ADVANCED ITEMS
		...
		event.getRegistry().register(
				new PickaxeItem(ModItemTier.EXAMPLE, 4, 3.0f, new Item.Properties().group(TutorialItemGroup.instance))
						.setRegistryName("example_pickaxe"));
		event.getRegistry().register(
				new ShovelItem(ModItemTier.EXAMPLE, 2, 7.0f, new Item.Properties().group(TutorialItemGroup.instance))
		...
		event.getRegistry().register(
				new HoeItem(ModItemTier.EXAMPLE, 10.0f, new Item.Properties().group(TutorialItemGroup.instance))
	...
	public enum ModArmorMaterial implements IArmorMaterial {
		TEST(TutorialMod.MOD_ID + ":test", 5, new int[] { 7, 9, 11, 7 }, 420, SoundEvents.ITEM_ARMOR_EQUIP_DIAMOND, 6.9F, () -> {
			return Ingredient.fromItems(ItemInit.example_item);
		});
		...
		private ModArmorMaterial(String nameIn, int maxDamageFactorIn, int[] damageReductionAmountIn,
				int enchantablilityIn, SoundEvent soundEventIn, float toughnessIn,
				Supplier<Ingredient> repairMaterialIn) {
			this.name = nameIn;
			this.maxDamageFactor = maxDamageFactorIn;
			this.damageReductionAmountArray = damageReductionAmountIn;
			this.enchantability = enchantablilityIn;
